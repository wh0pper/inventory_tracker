...
post('/shoe/:id/add_store') do
  ...
  quantity = params['send_quantity'].to_i
  @shoe.stores.push(new_store)
  @shoe.inventories.where({:store_id => new_store.id}).update({:inventory => quantity})
  erb:shoe
end

patch('/shoe/:id/edit_shoe') do
  shoe_update = Shoe.find(params['id'].to_i)
  shoe_update.update({:name => params['update_name'], :price => params['update_price'].to_i})
  redirect to "/shoe/#{shoe_update.id}"
end
